# File: HJ_Habibi_040913_Sep13_Analysis1_2.R
...
xtabs(~ oExp.lumi$group + oExp.lumi$ethnicity)
...
i = g | i
## OR
i = grepl('^Asymptomatic, infected$', x = oExp.lumi$group)
g = grepl('^0$', oExp.lumi$day)
i = g | i
## OR
...
g = grepl('^0$', oExp.lumi$day)
i = g | i
#i = i & grepl('^CHINESE$', x = oExp.lumi$ethnicity)
...
table(dfSamples[ , 'group'], dfSamples[, 'ethnicity'])

## create grouping factors by data
fSamples = paste0('D', dfSamples$day)
...
fSamples = factor(fSamples, levels=c('D0', 'D1', 'D3', 'D7', 'D10', 'D14', 'D28'))
levels(fSamples)
table(fSamples)
...
for (i in seq_along(n)) {
  ...
  f_plotVolcano(dfGenes, paste(names(n[i])), fc.lim = c(-2.0, 2.0))
}
...
for (i in seq_along(n)) {
  ...
  f = paste('Results/', 'Significant_genes_at_10pcFDR_cliCold', names(n[i]), '.csv', sep='')
...
write.csv(dfTopGenes.cent, file='Results/Top_Centrality_Genes_sep13.csv')
...
save(oGr, file='Objects/sep13_graph.rds')
