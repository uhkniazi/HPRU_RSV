# File: HJ_Habibi_200213_Apr13_Days_Symptoms_3.R
...
i = !grepl('^Infected no cold$', x = oExp.lumi$Symptoms)
# group days 0 and 1 into other group
...
# infected no cold together merged and split into 3 days
i = grepl('^Infected no cold$', x = oExp.lumi$Symptoms)
...
# sanity check
table(dfSamples[i & (i2 | i3 | i4), 'Symptoms'], dfSamples[i & (i2 | i3 | i4), 'Day'])
fSamples[i & i2] = paste0('NoCold', '3.5')
fSamples[i & i3] = paste0('NoCold', '7.10')
fSamples[i & i4] = paste0('NoCold', dfSamples[i & i4, 'Day'])
table(fSamples)

fSamples = factor(fSamples, levels = c('Base', 'NoCold3.5', 'NoCold7.10', 'NoCold28'))
table(fSamples)
...
######### Volcano plots
# plot volcano plots
n = (which(sapply(lSigGenes.adj, length) > 5)) + 1
...
for (i in seq_along(n)) {
  ...
  y.cut = quantile(p.val[c], probs=0.75)
...
for (i in seq_along(n)) {
  ...
  f = paste('Results/', 'Significant_genes_at_10pcFDR_Base_vs_InfNoCold', names(n[i]), '.csv', sep='')
...
m = getSignificantClusters(oGr, t(mCounts), fGroups, bStabalize = F)
dfCluster = getClusterMapping(oGr)
colnames(dfCluster) = c('gene', 'cluster')
rownames(dfCluster) = dfCluster$gene
...
ig = f_igCalculateVertexSizesAndColors(ig, t(mCounts), fGroups, bColor = T, iSize=50)
n = V(ig)$name
lab = f_dfGetGeneAnnotation(n)
V(ig)$label = as.character(lab$SYMBOL)
set.seed(1)
plot(ig, vertex.label.cex=0.14, layout=layout_with_fr, vertex.frame.color='darkgrey', edge.color='lightgrey', main='Cold7.10 vs NC')
legend('topright', legend = c('Underexpressed', 'Overexpressed'), fill = c('lightblue', 'pink'))
...
ig = getClusterSubgraph(oGr, csClust)
#fG = factor(fGroups, levels = c('NC', 'Cold7.10', 'Cold3.5'))
ig = f_igCalculateVertexSizesAndColors(ig, t(mCounts), fG, bColor = T, iSize=50)
n = V(ig)$name
lab = f_dfGetGeneAnnotation(n)
V(ig)$label = as.character(lab$SYMBOL)
set.seed(1)
plot(ig, vertex.label.cex=0.14, layout=layout_with_fr, vertex.frame.color='darkgrey', edge.color='lightgrey', main='Cold3.5 vs NC')
legend('topright', legend = c('Underexpressed', 'Overexpressed'), fill = c('lightblue', 'pink'))
...
write.csv(dfCluster, file='Results/Top_Clusters_HJ_Habibi_200213_Apr13_NC_vs_Cold.csv')
